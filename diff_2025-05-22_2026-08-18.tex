...
%DIF DEL flat_2025-05-22.tex   Tue Aug 18 12:14:26 2026
%DIF ADD flat_2026-08-18.tex   Tue Aug 18 12:21:31 2026
...
\DIFdelbegin \DIFdel{Following }\DIFdelend \DIFaddbegin \DIFadd{The following }\DIFaddend generalizations are considered: 
...
\begin{equation*}
	...
	\eqfs
\end{equation*}
Let $Y_1, \dots, Y_n$ be independent random variables with the same distribution as $Y$. Choose $(\epsilon_n)_{n\in\N}\subset [0,\infty)$ with $\epsilon_n \xrightarrow{n\to\infty}0$. Let $M_n := \epsilon_n\text{-}\argmin_{q\in\mc Q} \frac{1}n \sum_{i=1}^n \mf c(Y_i, q)$. Our goal is to show almost sure convergence of elements in $M_n$ to elements in $M$. 
\begin{remark}
...
\cite{huckemann11} shows almost sure convergence in \DIFdelbegin \DIFdel{one-side }\DIFdelend \DIFaddbegin \DIFadd{one-sided }\DIFaddend Hausdorff distance as well as in outer limit for generalized Fréchet means. Our results for $\mf c$-Fréchet means require slightly less strict assumptions, see \autoref{thm:epi} and \autoref{thm:consistency}, which make them applicable in a larger class of settings and allows us to derive our results for $H$- and $\alpha$-Fréchet means with minimal moment assumptions.
...
When $M$ is \DIFdelbegin \DIFdel{singleton }\DIFdelend \DIFaddbegin \DIFadd{a singleton, }\DIFaddend a quantitative version (rates of convergence) of the results presented in this article is given in \cite{schoetz19}.
...
All strong laws in the main part of this article build upon \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite[Theorem 1.1]{korf01} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[Theorems 1.1 and 8.2]{korf01} }\hskip0pt%DIFAUXCMD
}\DIFaddend -- \DIFdelbegin \DIFdel{a deep convergence result }\DIFdelend \DIFaddbegin \DIFadd{deep convergence results }\DIFaddend for functions $\MS \to \R$. In appendix \ref{sec:alt}, we show a different route to a strong law in \DIFdelbegin \DIFdel{one-side }\DIFdelend \DIFaddbegin \DIFadd{one-sided }\DIFaddend Hausdorff distance. This is illustrative, but requires slightly stricter assumptions. 
...
\begin{proof}
		...
		\end{equation*}
\end{proof}
It is illustrative to compare this result with \autoref{thm:alt:convOfMini}, which shows that a stronger notion of convergence for functions -- \DIFdelbegin \DIFdel{convergences }\DIFdelend \DIFaddbegin \DIFadd{convergence }\DIFaddend uniformly on bounded sets -- yields convergence of sets of minimizers in one-sided Hausdorff distance, which is a stronger notion of convergence of sets as the next theorem shows.
...
Choose $(\epsilon_n)_{n\in\N}\subset [0,\infty)$ with $\epsilon_n \xrightarrow{n\to\infty}0$. Set $M_n = \epsilon_n\text{-}\argmin_{q\in\mc Q} \frac{1}n \sum_{i=1}^n \mf c(Y_i, q)$.
...
\end{theorem}
\begin{proof}
Define $F(q) = \Ex{\mf c(Y, q)}$, $F_n(q) = \frac{1}n\sum_{i=1}^n \mf c(Y_i, q)$. By \textsc{Integrable}, $F(q)<\infty$.
...
\begin{itemize}
	...
	\item \textsc{SampleHeineBorel}: Almost surely\DIFaddbegin \DIFadd{, the }\DIFaddend following is true: There is $N_0\in\N$ such that every closed and bounded subset of $\bigcup_{n\geq N_0} M_n$ is compact.
	...
	\begin{align*}
		...
		\eqcm\\
		\mf a^+_n \psi^+(\ol qo) - \mf a^-_n \psi^-(\ol qo) &\leq \frac{1}n \sum_{i=1}^n \mf c(Y_i, q)
	\end{align*}
...
\begin{itemize}
\item 
	\DIFdelbegin \DIFdel{Following }\DIFdelend \DIFaddbegin \DIFadd{The following }\DIFaddend implications hold:
	\begin{align*}
...
\end{theorem}
\begin{proof}
The proof consists of \DIFaddbegin \DIFadd{the }\DIFaddend following steps:
\begin{enumerate}
...
Define $F(q) = \Ex{\mf c(Y, q)}$, $F_n(q) = \frac{1}n\sum_{i=1}^n \mf c(Y_i, q)$.
We want to show that there is a bounded set $B_1 \subset \mc Q$ \DIFdelbegin \DIFdel{such that $F(q) \geq F(m) + 1$ and $F_n(q) \geq F_n(m) + 1$ }\DIFdelend \DIFaddbegin \DIFadd{and a random variable $N_1$ such that $F(q) \geq \inf F + 1$ and $F_n(q) \geq \inf F_n + 1$ }\DIFaddend for all $q\in \mc Q \setminus B_1$ and \DIFdelbegin \DIFdel{$m \in M$}\DIFdelend \DIFaddbegin \DIFadd{all $n \geq N_1$}\DIFaddend . If $\mc Q$ is bounded, we can take $B_1 = \mc Q$ \DIFaddbegin \DIFadd{and $N_1 = 1$}\DIFaddend . Assume $\mc Q$ is not bounded.

Let \DIFdelbegin \DIFdel{$m \in M$}\DIFdelend \DIFaddbegin \DIFadd{$o\in\mc Q$ from }\textsc{\DIFadd{LowerBound}}\DIFaddend . By \textsc{UpperBound}, \DIFdelbegin \DIFdel{$F(m) < \infty$. Let $o\in\mc Q$ from }\textsc{\DIFdel{LowerBound}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$F(o) < \infty$. For $\mf b^+, \mf b^- \in(0,\infty)$ and $\delta \geq 0$ set
}\begin{equation*}
	\DIFadd{G_{\mf b^+, \mf b^-}(\delta)
	:=
	\inf_{t \geq \delta} \br{\mf b^+ \psi^+(t) - \mf b^- \psi^-(t)}
	\eqfs
}\end{equation*}
\DIFadd{As $\psi^\pm \geq 0$, we have $\mf b^+ \psi^+(t) - \mf b^- \psi^-(t) \geq \max(1,\psi^-(t)) \br{\mf b^+ \frac{\psi^+(t)}{\max(1,\psi^-(t))} - \mf b^-}$ for all $t\geq0$. Since $\psi^+(t)/\max(1, \psi^-(t))\xrightarrow{t\to\infty}\infty$ and $\max(1,\psi^-(t))\geq1$, this implies $G_{\mf b^+, \mf b^-}(\delta) \xrightarrow{\delta\to\infty} \infty$. Note that no monotonicity of $\psi^+$ or $\psi^-$ is assumed, so it is essential to take the infimum over all $t\geq\delta$ here}\DIFaddend .
Due to \textsc{LowerBound} \DIFdelbegin \DIFdel{, $F(q) \geq \mf a^+ \psi^+(\delta) - \mf a^- \psi^-(\delta) \geq  F(m) + 2$ }\DIFdelend \DIFaddbegin \DIFadd{and $\ol qo \geq \delta$ for $q\in\mc Q\setminus\ball_\delta(o)$, we obtain $F(q) \geq G_{\mf a^+, \mf a^-}(\delta)$ }\DIFaddend for all $q\in\mc Q\setminus \ball_\delta(o)$\DIFdelbegin \DIFdel{and $\delta$ large enough.
This holds for all $m\in M$ as $F(m)$ does not change with $m$. We set $B_1 = \ball_\delta(o)$. For $F_n$, it holds $F_n(m) \xrightarrow{n\to\infty}_{\ms{a.s.}} F(m)$ and $\inf_{q\in\mc Q \setminus B_1} F_n(q) \geq \mf a^+_n \psi^+(\delta) - \mf a^-_n \psi^-(\delta)$ with $\mf a^+_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^+$ }\DIFdelend \DIFaddbegin \DIFadd{.
As $\mf a^+_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^+ > 0$ }\DIFaddend and $\mf a^-_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^-$\DIFdelbegin \DIFdel{.
Thus}\DIFdelend , there is a random variable $N_1$ such that almost surely \DIFdelbegin \DIFdel{$F_n(q) \geq F_n(m) + 1$ }\DIFdelend \DIFaddbegin \DIFadd{$\mf a^+_n \geq \frac{1}{2} \mf a^+$ and $\mf a^-_n \leq \mf a^- +1$ for all $n\geq N_1$; enlarging $N_1$ if necessary, we may also assume $F_n(o) \leq F(o) + 1$ }\DIFaddend for all $n \geq N_1$, \DIFaddbegin \DIFadd{which is possible by the strong law of large numbers and }\textsc{\DIFadd{Integrable}}\DIFadd{. Hence, almost surely, $F_n(q) \geq G_{\frac{1}{2}\mf a^+, \mf a^-+1}(\delta)$ for all $n\geq N_1$ and all $q\in\mc Q\setminus \ball_\delta(o)$.
As $G_{\mf b^+, \mf b^-}(\delta)\xrightarrow{\delta\to\infty}\infty$, we can choose $\delta$ large enough such that $G_{\mf a^+, \mf a^-}(\delta) \geq F(o) + 1$ and $G_{\frac{1}{2}\mf a^+, \mf a^-+1}(\delta) \geq F(o) + 2$. We set $B_1 = \ball_\delta(o)$.
Then, almost surely, $F(q) \geq F(o) + 1 \geq \inf F + 1$ and $F_n(q) \geq F(o) + 2 \geq F_n(o) + 1 \geq \inf F_n + 1$ for all }\DIFaddend $q\in \mc Q \setminus B_1$ \DIFdelbegin \DIFdel{, }\DIFdelend and \DIFdelbegin \DIFdel{$m\in M$}\DIFdelend \DIFaddbegin \DIFadd{$n \geq N_1$. In particular, $M \subset B_1$ and, for $\epsilon_n < 1$ and $n \geq N_1$, also $M_n \subset B_1$}\DIFaddend .
...
Let $Y_1, \dots, Y_n$ be independent random variables with the same distribution as $Y$.
Choose $(\epsilon_n)_{n\in\N}\subset [0,\infty)$ with $\epsilon_n \xrightarrow{n\to\infty}0$. Set $M_n = \epsilon_n\text{-}\argmin_{q\in\mc Q} \frac{1}n \sum_{i=1}^n (H(\ol {Y_i}q) - H(\ol {Y_i}o))$.
\begin{assumptions}\mbox{ }
	\begin{itemize}
...
\end{corollary}
\begin{proof}
	We check the conditions of \autoref{thm:epi}\DIFaddbegin \DIFadd{, with }\textsc{\DIFadd{IntegrableInf}} \DIFadd{weakened as described below}\DIFaddend . \textsc{Polish} is an assumption. 
	...
	\DIFdelbegin \DIFdel{To show }\DIFdelend \DIFaddbegin \DIFadd{Of }\DIFaddend \textsc{IntegrableInf}\DIFaddbegin \DIFadd{, we only need the local version
	}\begin{equation*}
		\DIFadd{\Ex*{\inf_{q\in B}\br{H(\ol Yq) - H(\ol Yo)}} > -\infty
		\quad\text{for every bounded set } B\subset\mc Q
		\eqcm
	}\end{equation*}
	\DIFadd{because one can follow the proof of }\autoref{thm:epi} \DIFadd{with \mbox{%DIFAUXCMD
\cite[Theorem 1.1]{korf01} }\hskip0pt%DIFAUXCMD
replaced by \mbox{%DIFAUXCMD
\cite[Theorem 8.2]{korf01}}\hskip0pt%DIFAUXCMD
, which requires only local instead of global inf-integrability.}\footnote{\DIFadd{\mbox{%DIFAUXCMD
\cite[Theorem 8.2]{korf01} }\hskip0pt%DIFAUXCMD
is applicable here: the local version of }\textsc{\DIFadd{IntegrableInf}} \DIFadd{implies that $\mf c$ is }\textit{\DIFadd{locally inf-integrable}} \DIFadd{in the sense of \mbox{%DIFAUXCMD
\cite[Definition 4.2]{korf01}}\hskip0pt%DIFAUXCMD
, as every $q\in\mc Q$ has a closed bounded neighborhood; a countable subset of $\mc Q\times\R$ that is dense in the epigraph of $F$ exists, as $\mc Q\times\R$ is separable by }\textsc{\DIFadd{Polish}} \DIFadd{and every subset of a separable metric space is separable; and the shift on the sequence space of the independent and identically distributed $(Y_i)_{i\in\N}$ is measure preserving and ergodic.}} \DIFadd{This is necessary here, as }\textsc{\DIFadd{IntegrableInf}} \DIFadd{itself is equivalent to $\Ex{H(\ol Yo)} < \infty$ -- since $H\geq0$ and $H(0)=0$, the infimum $\inf_{q\in\mc Q}\br{H(\ol yq) - H(\ol yo)} = -H(\ol yo)$ is attained at $q=y$ -- which is more restrictive than }\textsc{\DIFadd{$h$-Moment}}\DIFadd{.
	To show the local version}\DIFaddend , we note that $H$ is non-decreasing and apply \autoref{lmm:nondec} (iii),
	\begin{align*}
		H(\ol yq) - H(\ol yo)
		&\geq 
	...
	\end{align*}
	due to \textsc{Additivity}. Furthermore, $H(\delta)  = \int_0^\delta h(x) \dl x \geq \frac{1}{2} \delta h(\frac{1}{2} \delta)$.
	\DIFaddbegin \DIFadd{Let $B \subset \mc Q$ be a bounded set and set $D := \sup_{q\in B} \ol qo < \infty$. As $H \geq 0$, the inequality above implies
	}\begin{equation*}
		\DIFadd{\inf_{q \in B} \br{H(\ol yq) - H(\ol yo)} \geq - 2 D\, h(\ol yo)
		\eqfs
	}\end{equation*}
	\DIFaddend With that, \textsc{$h$-Moment} implies \DIFaddbegin \DIFadd{the local version of }\DIFaddend \textsc{IntegrableInf}.
	Thus, \DIFaddbegin \DIFadd{the conclusion of }\DIFaddend \autoref{thm:epi} \DIFdelbegin \DIFdel{can be applied}\DIFdelend \DIFaddbegin \DIFadd{holds}\DIFaddend .
\end{proof}
...
\end{corollary}
\begin{proof}
	We check the conditions of \autoref{thm:consistency}. \textsc{SampleHeineBorel} and \textsc{Polish} are assumptions of the corollary. \textsc{LowerSemiContinuity} and \DIFaddbegin \DIFadd{the local version of }\DIFaddend \textsc{IntegrableInf} are shown in the proof of \autoref{cor:nondec:epi}\DIFaddbegin \DIFadd{; }\textsc{\DIFadd{IntegrableInf}} \DIFadd{enters the proof of }\autoref{thm:consistency} \DIFadd{only through }\autoref{thm:epi} \DIFadd{in step 1, so that -- as in the proof of }\autoref{cor:nondec:epi} \DIFadd{-- its local version together with \mbox{%DIFAUXCMD
\cite[Theorem 8.2]{korf01} }\hskip0pt%DIFAUXCMD
suffices}\DIFaddend .
	...
	\begin{align*}
		...
		H(\delta)  &\geq \frac{1}{2} \delta h\brOf{\frac{1}{2} \delta}
		\eqfs
	\end{align*}
	The first inequality together with \textsc{$h$-Moment} implies \textsc{UpperBound}.
	For \textsc{LowerBound}, we use the second inequality. We set $\psi^+(\delta) = 	b^{-1} H(\delta)$, $\psi^-(\delta) = 2 \delta$, $\mf a^+=\mf a_n^+ = 1$, \DIFdelbegin \DIFdel{$\mf a^{-} = \Ex{h(\ol Yo)}$, and $\mf a^-_n = \frac{1}n \sum_{i=1}^n h(\ol {Y_i}o)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mf a^{-} = \Ex{h(\ol Yo)} + 1$, and $\mf a^-_n = \frac{1}n \sum_{i=1}^n h(\ol {Y_i}o) + 1$ }\DIFaddend with $\mf a^-_n\xrightarrow{n\to\infty}_{\ms{a.s.}}\mf a^-$ due to \textsc{$h$-Moment}. \DIFaddbegin \DIFadd{The added constant $1$ only weakens the lower bounds, as $\psi^-\geq0$, and it guarantees $\mf a^-\in(0,\infty)$ also in the degenerate case $\Ex{h(\ol Yo)} = 0$. }\DIFaddend Because of the third inequality, $\psi^+(\delta)/\psi^-(\delta) \geq \frac{1}{4} b^{-1} h(\frac{1}{2} \delta) \xrightarrow{\delta\to\infty} \infty$ by \textsc{InfiniteIncrease}.
\end{proof}
 \section{Strong Laws for $\alpha$-Fréchet Mean Sets}\label{sec:power}
Let $(\mc Q, d)$ be a metric space. Let $(\Omega, \Sigma, \Pr)$ be a probability space that is silently underlying all random variables in this section. Let $Y$ be a random variable with values in \DIFdelbegin \DIFdel{$\mc Y$}\DIFdelend \DIFaddbegin \DIFadd{$\mc Q$}\DIFaddend .
...
Let $Y_1, \dots, Y_n$ be independent random variables with the same distribution as $Y$.
Choose $(\epsilon_n)_{n\in\N}\subset [0,\infty)$ with $\epsilon_n \xrightarrow{n\to\infty}0$. Set $M_n =\epsilon_n\text{-}\argmin_{q\in\mc Q} \frac{1}n \sum_{i=1}^n (\ol {Y_i}q^\alpha - \ol{Y_i}o^\alpha)$.
...
\end{proof}
\begin{remark}
	 For convergence, we need the $(\alpha-1)$-moment to be finite in the case of $\alpha \geq 1$. \cite[Corollary 5]{schoetz19} shows that in metric spaces with nonnegative curvature the typical parametric rate of convergence $n^{-\frac{1}{2}}$ is obtained for $\alpha$-Fréchet means assuming the $2(\alpha-1)$-moment to be finite in the case of $\alpha\in[1,2]$ under some further conditions.
\end{remark}
 \begin{appendix}
\section{Example: The Set of Medians}\label{sec:median}
Let $s\in\N$. Consider the metric space $(\R^s, d_1)$, where $d_1(q,p) = \normof{q-p}_1 = \sum_{j=1}^s \abs{q_j-p_j}$. The power Fréchet mean with $\alpha=1$ in this space is equivalent to the standard ($\alpha=2$) Fréchet mean in $(\R^s, d_1^{\frac{1}{2}})$. For $s=1$ it is equal to the median.
Let $Y = (Y^1,\dots, Y^s)$ be a random vector in $\R^s$ such that $\Pr(Y^k=0)=\Pr(Y^k=1)=\frac{1}{2}$ for $k=1,\dots, s$ and $Y^1,\dots, Y^s$ are independent. Let $Y_1, Y_2,\dots$ be independent and identically distributed copies of $Y$. Let $M = \argmin_{q\in\R^s}\Ex{d_1(Y, q)}$ be the Fréchet mean set of $Y$ and $M_n = \epsilon_n\text{-}\argmin_{q\in\R^s}\frac{1}n\sum_{i=1}^n d_1(Y_i, q)$ its sample version.
...
Define $p_n := \frac{1}n \sum_{i=1}^n Y_i$. Then the empirical objective function is $F_n(q) := p_n\abs{1-q}+(1-p_n)\abs{q}$, i.e., the sample Fréchet mean set is $M_n = \argmin_{q\in\R} F_n(q)$.
If $n$ is odd, then either $M_n = \{0\}$ or $M_n = \{1\}$ holds. The same is true for an even value of $n$ except when $p_n=\frac{1}{2}$, in which case $M_n=[0,1]$.
...
By \autoref{lmm:products}, $M$ is just the Cartesian product of the median sets in each dimension, i.e., $M = [0,1]^s$ (this is not to be confused with the geometric median, which is the Fréchet mean with respect to the square root of the Euclidean norm). Similarly, $M_n = \bigtimes_{k=1}^s M_n^k$ decomposes into the sample Fréchet mean sets $M_n^k$ of each dimension $k=1,\dots,s$. It holds $M_n^k = [0,1]$ if and only if the respective value of $p_n^k := \frac{1}n \sum_{i=1}^n Y_i^k$ is equal to $\frac{1}{2}$, i.e., if and only if the symmetric simple random walk $S_n^k := \sum_{i=1}^n (2Y_i^k-1)$ hits $0$. Let $N = \#\Set{n\in\N\given S_n^1=\dots=S_n^s=0}$. Let \DIFdelbegin \DIFdel{$A\subset \R$ and $B_n\subset \R$ }\DIFdelend \DIFaddbegin \DIFadd{$A\subset \R^s$ and $B_n\subset \R^s$ }\DIFaddend for all $n\in\N$. If $A\subset B_n$ for infinitely many $n$, then $A \subset \outerlim_{n\to\infty} B_n$. Thus, we want to know whether $N$ is finite or infinite. This is answered by \textit{Pólya's Recurrence Theorem} \cite{polya21}. It implies that for $s \in \{1, 2\}$, $N = \infty$ almost surely. Furthermore, if $s\geq 3$, then $N < \infty$ almost surely.
To find which points are not element of the outer limit of sample Fréchet mean sets, note \DIFaddbegin \DIFadd{the }\DIFaddend following fact: For an open subset \DIFdelbegin \DIFdel{$A\subset \R$, if $A \subset \R \setminus B_n$ }\DIFdelend \DIFaddbegin \DIFadd{$A\subset \R^s$, if $A \subset \R^s \setminus B_n$ }\DIFaddend for all but finitely many $n$, then \DIFdelbegin \DIFdel{$A \subset \R \setminus \outerlim_{n\to\infty} B_n$}\DIFdelend \DIFaddbegin \DIFadd{$A \subset \R^s \setminus \outerlim_{n\to\infty} B_n$}\DIFaddend .
...
At least one of $0$ and $1$ is a minimizer of $F_n(q)=p_n\abs{1-q}+(1-p_n)\abs{q}$ and \DIFdelbegin \DIFdel{the $F_n(q)$ }\DIFdelend \DIFaddbegin \DIFadd{$F_n$ }\DIFaddend is linear on $[0,1]$. Thus, $[0,1] \subset M_n$ if and only if  $\epsilon_n \geq |F_n(0)-F_n(1)|$. This is equivalent to
 \DIFdelbegin \DIFdel{$|p_n-\frac{1}{2}| < \frac{1}{2}\epsilon_n$.
By Markov}\DIFdelend \DIFaddbegin \DIFadd{$|p_n-\frac{1}{2}| \leq \frac{1}{2}\epsilon_n$.
As $Y_1,\dots, Y_n$ are independent with values in $[0,1]$ and $\Ex{p_n} = \frac{1}{2}$, Hoeffding}\DIFaddend 's inequality \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[Theorem 2]{hoeffding63} }\hskip0pt%DIFAUXCMD
yields
}\DIFaddend \begin{equation*}
	\DIFdelbegin %DIFDELCMD < \PrOf{\abs{p_n-\frac{1}{2}} \geq \frac{1}{2}\epsilon_n} %%%
\DIFdelend \DIFaddbegin \PrOf{\abs{p_n-\frac{1}{2}} > \frac{1}{2}\epsilon_n} \DIFaddend \leq \DIFdelbegin \DIFdel{\frac{n^{-3} \Ex*{(Y-\frac{1}2)^4}}{2^{-4}\epsilon_n^4}
	}\DIFdelend \DIFaddbegin \DIFadd{2 \exp}\brOf{-\tfrac12 n \epsilon_n^2}
	\DIFaddend \eqfs
\end{equation*}
For $\epsilon_n = n^{-\frac{1}{4}}$, we obtain
\begin{equation*}
	\sum_{n=1}^\infty \DIFdelbegin %DIFDELCMD < \PrOf{\abs{p_n-\frac{1}{2}} \geq \frac{1}{2}\epsilon_n}  %%%
\DIFdelend \DIFaddbegin \PrOf{\abs{p_n-\frac{1}{2}} > \frac{1}{2}\epsilon_n}  \DIFaddend \leq \sum_{n=1}^\infty \DIFdelbegin \DIFdel{n^{-2} }\DIFdelend \DIFaddbegin \DIFadd{2\exp}\brOf{-\tfrac12 \sqrt n} \DIFaddend < \infty
	\eqfs
\end{equation*}
The Borel--Cantelli lemma implies that almost surely and for all $n$ large enough,  \DIFdelbegin \DIFdel{$|p_n-\frac{1}{2}| < \frac{1}{2}\epsilon_n$ }\DIFdelend \DIFaddbegin \DIFadd{$|p_n-\frac{1}{2}| \leq \frac{1}{2}\epsilon_n$ }\DIFaddend and thus, $[0,1] \subset M_n$. 
...
\begin{theorem}\label{thm:alt:convOfMini}
	...
	\begin{equation*}
		...
		0
		\DIFaddbegin \DIFadd{\eqfs
	}\DIFaddend \end{equation*}
	\DIFdelbegin \DIFdel{and
	}\DIFdelend \DIFaddbegin \DIFadd{If, in addition, $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n$ large enough, then also
	}\DIFaddend \begin{equation*}
		\inf f_n \xrightarrow{n\to\infty} \inf f
		\eqfs
	\end{equation*}
\end{theorem}
\DIFaddbegin \DIFadd{The additional condition for the convergence of the infima holds, e.g., if $\epsilon_n > 0$ and $\inf f_n > -\infty$. It cannot be dropped: on $\MS = \R$, let $\epsilon_n = 0$, $f(x) = x^2$, and $f_n(x) = f(x)$ for $\abs x \leq n$, $f_n(x) = -1 + \abs x^{-1}$ for $\abs x > n$. Then $f_n\ubsconv{n}f$, $f$ has approachable minimizers, and $\argmin f_n = \emptyset$, so that $(\argmin f_n)_{n\in\N}$ is eventually bounded and $d_\subset(\argmin f_n, \argmin f) = 0$. But $\inf f_n = -1$ for all $n\in\N$, whereas $\inf f = 0$.
		...
		Finally, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{assume $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n$ large enough and let us }\DIFaddend show the convergence of the infima.
		\DIFdelbegin \DIFdel{We already know $\inf f_n \leq \inf f + \epsilon$ for all $\epsilon > 0$ }\DIFdelend \DIFaddbegin \DIFadd{As $\argmin f \neq \emptyset$, we may pick $y \in \argmin f$. From $f_n(y) \xrightarrow{n\to\infty} f(y) = \inf f$ }\DIFaddend and \DIFaddbegin \DIFadd{$\inf f_n \leq f_n(y)$, we obtain $\limsup_{n\to\infty} \inf f_n \leq \inf f$.
		Assume $\liminf_{n\to\infty} \inf f_n < \inf f$. Then there is $\eta > 0$ such that $\inf f_n < \inf f - 2\eta$ for infinitely many }\DIFaddend $n$\DIFdelbegin \DIFdel{large enough. If $\inf f_n \xrightarrow{n\to\infty} \inf f$ does not hold, there is a sequence $x_n \in \epsilon_n\text{-}\argmin f_n$ }\DIFdelend \DIFaddbegin \DIFadd{. Let $n_3$ }\DIFaddend and \DIFdelbegin \DIFdel{$\epsilon > 0$ such that $f_n(x_n) < \inf f - \epsilon$ for all $n$ large enough. As before, because of eventual boundedness and uniform convergence on bounded sets, we have $\sup_{k\in\N}\abs{f_n(x_k) - f(x_k)} \xrightarrow{n\to\infty} 0$. Therefore, for }\DIFdelend \DIFaddbegin \DIFadd{$B = \bigcup_{n\geq n_3} \epsilon_n\text{-}\argmin f_n$ be as above. As $f_n \xrightarrow{n\to\infty}_{\ubs} f$ and $\diam(B)<\infty$, there is $n_5\in\N$ such that $\sup_{x\in B}\abs{f_n(x) - f(x)} \leq \frac{1}{2} \eta$ for all $n \geq n_5$. As $\epsilon_n \xrightarrow{n\to\infty} 0$, there is $n_6\in\N$ such that $\epsilon_n \leq \eta$ for all $n\geq n_6$. By assumption, there is $n_7\in\N$ such that $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for }\DIFaddend all \DIFdelbegin \DIFdel{$\epsilon >0$ we have $f(x_n) \leq f_n(x_n) + \epsilon$ for $n$ large enough, which contradicts $f_n(x_n) < \inf f - \epsilon$}\DIFdelend \DIFaddbegin \DIFadd{$n \geq n_7$. For every one of the infinitely many $n \geq \max(n_3, n_5, n_6, n_7)$ with $\inf f_n < \inf f - 2\eta$, choose $x_n \in \epsilon_n\text{-}\argmin f_n$. Then $x_n \in B$ and
		}\begin{equation*}
			\DIFadd{f(x_n) \leq f_n(x_n) + \tfrac12\eta \leq \inf f_n + \epsilon_n + \tfrac12\eta < \inf f - 2\eta + \eta + \tfrac12\eta < \inf f
			\eqcm
		}\end{equation*}
		\DIFadd{which is a contradiction. Hence, $\inf f_n \xrightarrow{n\to\infty} \inf f$}\DIFaddend .
\end{proof}
...
\begin{enumerate}[label=(\roman*)]
	...
	\item
	Let $f,f_n\colon \N_0 \to \R$, $f(0) = 0$, $f(i) = \frac{1}i$, $f_n(i) = f(i)\indOfEvent{i < n}$, and set $d(i,j) = 1$  for $i\neq j$.
...
\end{theorem}
\begin{proof}
Define $F(q) = \Ex{\mf c(Y, q)}$, $F_n(q) = \frac{1}n\sum_{i=1}^n \mf c(Y_i, q)$.  The proof consists of \DIFaddbegin \DIFadd{the }\DIFaddend following steps:
\begin{enumerate}
...
\begin{equation*}
	\sup_{q\in B} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q) - \Ex{f(Y, q)}} \xrightarrow{n\to\infty}_{\ms{a.s.}} 0 
	\eqfs
\end{equation*}
\end{theorem}
\begin{proof}
Let $\epsilon > 0$. As $B$ is compact, \DIFaddbegin \DIFadd{the open cover $(\ball_\epsilon(q))_{q \in B}$ of $B$ has a finite subcover, i.e., }\DIFaddend there is a finite set \DIFdelbegin \DIFdel{$\cb{q_1, \dots, q_k} \subset \mc Q$ }\DIFdelend \DIFaddbegin \DIFadd{$\cb{q_1, \dots, q_k} \subset B$ }\DIFaddend such that $B \subset \bigcup_{\ell=1}^k\ball_\epsilon(q_\ell)$. We split the supremum,
\begin{align*}
	&\sup_{q\in B} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q) - \Ex{f(Y, q)}}
	\\&\leq
	\sup_{\ell \in \cb{1,\dots, k}} 
	\sup\DIFdelbegin \DIFdel{_{q\in \ball_\epsilon(q_\ell)}  
		}\DIFdelend \DIFaddbegin \DIFadd{_{q\in B \cap \ball_\epsilon(q_\ell)}
		}\DIFaddend \abs{\frac{1}n \sum_{i=1}^n \br{f(Y_i, q) - f(Y_i, q_\ell)} - \Ex{f(Y, q) - f(Y, q_\ell)}}
	\\&\quad+ 
	\sup_{\ell \in \cb{1,\dots, k}} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q_\ell) - \Ex{f(Y, q_\ell)}}
	\eqfs
\end{align*}
For the second summand, by the standard strong law of large numbers applied to each $\ell\in\{1,\dots,k\}$ with $\Ex{Z} < \infty$, 
\begin{align*}
\sup_{\ell \in \cb{1,\dots, k}} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q_\ell) - \Ex{f(Y, q_\ell)}} \xrightarrow{n \to \infty}_{\ms{a.s.}} 0
\eqfs
\end{align*}
...
\begin{align*}
	...
	\\& \leq
	\frac{1}n \sum_{i=1}^n \sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y_i, q) - f(Y_i, p)} 
	+ 
...
\begin{equation}\label{eq:ulln:as_limsup}
	\PrOf{
		\limsup_{n\to\infty} \sup_{q\in B} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q) - \Ex{f(Y, q)}}
		\leq
		a_\epsilon
	} = 1
	\eqcm
...
\begin{align*}
	\lim_{\epsilon\searrow0} 	\DIFdelbegin %DIFDELCMD < \Ex*{\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, p) - f(Y, p)} }
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \Ex*{\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, q) - f(Y, p)} }
	\DIFaddend &=
	\DIFdelbegin %DIFDELCMD < \Ex*{\lim_{\epsilon\searrow0}\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, p) - f(Y, p)} }
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \Ex*{\lim_{\epsilon\searrow0}\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, q) - f(Y, p)} }
	\DIFaddend =
	0
	\eqfs
\end{align*}
...
\item $\frac{1}{2} h(x) + \frac{1}{2} h(y) \leq h (x+y) \leq b \br{h(x) + h(y)}$,
...
\item As $h$ is non-decreasing, $\max(h(x), h(y)) \leq h(x+y) \leq \max(h(2x), h(2y))$.
	By the definition of $b$ and with $\frac{1}{2}(u+v) \leq \max(u,v) \leq u+v$ for $u,v \geq 0$ the claim follows.
\item 
	...
	Hence, $f(x, y) \geq f(x, 0) = 0$, as \DIFdelbegin \DIFdel{$H(y) = 0$}\DIFdelend \DIFaddbegin \DIFadd{$H(0) = 0$}\DIFaddend .
...
\begin{proof}
	...
	Let $z_1 = 0$ and $z_{n+1} = \inf \cbOf{x\geq z_n +1 \,\big\vert\, 1-F(x) \leq \frac{1}n}$. As $F(x)\xrightarrow{x\to\infty} 1$, $z_n < \infty$. Furthermore, $z_{n+1}-z_n \geq 1$. Moreover, as $X$ is not almost surely bounded, $1-F(x) > 0$ for all $x\geq 0$. Set 
	\begin{align*}
	...
	}\DIFdelend \DIFaddbegin \DIFadd{As $1-F$ is non-increasing and right-continuous, the definition of $z_n$ implies $1-F(t) \leq 1-F(z_n) \leq \frac{1}{n-1}$ for all $t\geq z_n$ and $n\geq2$. Thus,
	}\DIFaddend \begin{align*}
		\lim_{x\to\infty} \tilde h(x)
		=
		\int_0^\infty \frac{g(t)}{1-F(t)} \dl t
		\geq
		\sum\DIFdelbegin \DIFdel{_{n=1}}\DIFdelend \DIFaddbegin \DIFadd{_{n=2}}\DIFaddend ^\infty \DIFaddbegin \DIFadd{(n-1) }\DIFaddend n\DIFdelbegin \DIFdel{^{-1} 
		}\DIFdelend \DIFaddbegin \DIFadd{^{-2}
		}\DIFaddend =
		\infty
		\eqfs
	\end{align*}
	Moreover, $\tilde h(x)$ is strictly increasing, as \DIFdelbegin \DIFdel{$g(t)\geq 0$ and $1-F(t) \geq 0$.
	The function }\DIFdelend \DIFaddbegin \DIFadd{$g(t) > 0$ and $1-F(t) > 0$.
	As $1-F$ is non-increasing and positive, the integrand $t \mapsto g(t)/(1-F(t))$ is bounded on every bounded interval. Hence, }\DIFaddend $\tilde h$ is \DIFdelbegin \DIFdel{continuously differentiable everywhere except at point $z_n$, $n\in\N$}\DIFdelend \DIFaddbegin \DIFadd{locally Lipschitz continuous and, in particular, absolutely continuous with $\tilde h\pr(t) = g(t)/(1-F(t))$ for almost all $t\geq0$}\DIFaddend . Thus,
	\begin{align*}
		\Ex{\tilde h(X)}
		&=
	...
	Furthermore, $a_{n+1} \leq a_n$ as $x_{n+1} \geq x_n + a_n^{-1}$. As $h$ is continuous and \DIFdelbegin \DIFdel{$a_n$ }\DIFdelend \DIFaddbegin \DIFadd{$a_{n+1}$ }\DIFaddend is the derivative of $h$ in the interval $(x_n, x_{n+1})$, $h$ is concave. 
...
Let $\alpha\geq1$.
Set $\mc Q := \bigtimes_{k=1}^K \mc  Q_k$ and $d \colon \mc Q \times \mc Q \to [0,\infty)$, $d(q,p) := (\sum_{k=1}^K d_k(q_k, p_k)^\alpha)^{\frac{1}{\alpha}}$. Then $(\mc Q, d)$ is a metric space. Let $Y = (Y^1, \dots, Y^K)$ be a tuple of random variables such that $Y^k$ has values in $\mc Q_k$ and $\Ex{d(Y,o)^{\alpha-1}} < \infty$ for an element $o\in\mc Q$. Let $M$ be the $\alpha$-Fréchet mean set of $Y$ in $(\mc Q, d)$, and $M^k$ be the $\alpha$-Fréchet mean set of $Y^k$ in $(\mc Q_k, d_k)$. Then $M$ is the Cartesian product of the sets $M^k$, i.e.,
\begin{equation*}
	M = \bigtimes_{k=1}^K M^k
	\eqfs
\end{equation*}
\end{lemma}
\autoref{lmm:products} can be proven by \DIFdelbegin \DIFdel{straight forward }\DIFdelend \DIFaddbegin \DIFadd{straightforward }\DIFaddend calculations.
...
\end{lemma}
\begin{proof}
	\DIFdelbegin \DIFdel{Let $(a_k)_{k\in\N}\subset \overline{A}$. We show that $(a_k)_{k\in\N}$ has a converging subsequence with limit $a \in \overline{A}$, which implies compactness of $\overline{A}$}\DIFdelend \DIFaddbegin \DIFadd{As $(\mc Q, d)$ is complete and $\overline A$ is closed, $\overline A$ is complete. Thus, it suffices to show that $\overline A$ is totally bounded, which is equivalent to $A$ being totally bounded}\DIFaddend .

	\DIFdelbegin \DIFdel{Let $\delta_n = 2^{-n}$. Define $B_n := \mc Q \setminus \ball_{\delta_n}(\mc Q \setminus A)$. The sets $B_n$ are closed and subsets of }\DIFdelend \DIFaddbegin \DIFadd{Assume }\DIFaddend $A$ \DIFaddbegin \DIFadd{is not totally bounded. Then there is $\epsilon > 0$ such that no finite collection of balls of radius $\epsilon$ covers $A$. Hence, we can recursively choose $a_1 \in A$ and $a_{k+1} \in A \setminus \bigcup_{j=1}^{k}\ball_\epsilon(a_j)$ for $k \in \N$. The resulting set $S = \Set{a_k \given k \in\N}$ is infinite and $\epsilon$-separated, i.e., $d(a_j, a_k) \geq \epsilon$ for all $j \neq k$. In particular, every ball of radius $\frac{1}{2}\epsilon$ contains at most one element of $S$}\DIFaddend . Thus, \DIFdelbegin \DIFdel{they are compact. Let $b_k^n\in\argmin_{b \in B_n} d(b, a_k)$.
Such an element exists as $B_n$ }\DIFdelend \DIFaddbegin \DIFadd{$S$ has no point of accumulation in $\mc Q$, which makes $S$ a closed subset of $\mc Q$, and the cover of $S$ by the balls $\ball_{\frac{1}{2}\epsilon}(a_k)$, $k\in\N$, has no finite subcover, which makes $S$ non-compact. As $S \subset A$, this contradicts the assumption of the lemma.
}

	\DIFadd{Hence, $A$ is totally bounded and $\overline A$ }\DIFaddend is compact.
\DIFdelbegin \DIFdel{Furthermore, $d(b_k^n, a_k) \leq \delta_n$. Define the subindex sequences $(k(n,\ell))_{\ell\in\N} \subset \N$ such that $k(0, \ell) = \ell$ and $(b^n_{k(n, \ell)})_{\ell\in\N}$ is a converging subsequence of $(b^n_{k(n-1, \ell)})_{\ell\in\N}$ with limit $b^n_{k(n, \ell)} \xrightarrow{\ell\to\infty}b^n_\infty$ and $d(b^n_{k(n, \ell)}, b^n_\infty) \leq \delta_\ell$.
	By the triangle inequality $d(b^{n_1}_k, b^{n_2}_k) \leq d(b^{n_1}_k, a_k) + d(b^{n_2}_k, a_k) \leq \delta_{n_1}+\delta_{n_2}$. Thus, $d(b^{n_1}_\infty, b^{n_2}_\infty) \leq \delta_{n_1}+\delta_{n_2}$, which makes $(b^{n}_\infty)_{n\in\N}$ a Cauchy-sequence.
	Define $a$ as its limit, i.
	e.
	, $b^{n}_\infty \xrightarrow{n\to\infty} a$.
	As $b^{n}_\infty\in B_n \subset\overline{A}$, also $a\in\overline{A}$.
	Finally, the triangle inequality yields
	}\begin{displaymath}
		\DIFdel{d(a_{k(n, n)}, a) \leq d(a_{k(n, n)}, b^n_{k(n, n)}) + d(b^n_{k(n, n)}, b^{n}_\infty) + d(b^{n}_\infty, a) \xrightarrow{n\to\infty}0\eqcm
	}\end{displaymath}%DIFAUXCMD
%DIFDELCMD < 	%%%
\DIFdel{i.e.
	, $(a_{k(n, n)})_{n\in\N}$ is subsequence of $(a_k)_{k\in\N}$ which converges in $\overline{A}$.
	}\DIFdelend \end{proof}
\DIFaddbegin \begin{remark}[Corrections to the published version]\label{rem:corrections}
	\DIFadd{Compared to the published version of this article, the present version corrects the following errors. The numbering of all statements is unchanged.
	}\begin{enumerate}[label=(\roman*)]
	\item
		\DIFadd{Step 2 of the proof of }\autoref{thm:consistency}\DIFadd{: the lower bound of }\textsc{\DIFadd{LowerBound}} \DIFadd{was evaluated at $\ol qo = \delta$, although no monotonicity of $\psi^+$ or $\psi^-$ is assumed, and the argument used an element $m\in M$ before $M \neq \emptyset$ had been established. The step is now carried out with $G_{\mf b^+, \mf b^-}(\delta) = \inf_{t\geq\delta}\br{\mf b^+ \psi^+(t) - \mf b^- \psi^-(t)}$ and with $\inf F$ and $\inf F_n$ in place of $F(m)$ and $F_n(m)$.
	}\item
		\DIFadd{Proofs of }\autoref{cor:nondec:epi} \DIFadd{and }\autoref{cor:nondec:onehaus}\DIFadd{: }\textsc{\DIFadd{$h$-Moment}} \DIFadd{does not imply }\textsc{\DIFadd{IntegrableInf}} \DIFadd{-- for the cost function of section \ref{sec:nondec}, }\textsc{\DIFadd{IntegrableInf}} \DIFadd{is equivalent to $\Ex{H(\ol Yo)} < \infty$. Only a local version of }\textsc{\DIFadd{IntegrableInf}} \DIFadd{is needed, which follows from }\textsc{\DIFadd{$h$-Moment}} \DIFadd{and which suffices when \mbox{%DIFAUXCMD
\cite[Theorem 1.1]{korf01} }\hskip0pt%DIFAUXCMD
is replaced by \mbox{%DIFAUXCMD
\cite[Theorem 8.2]{korf01} }\hskip0pt%DIFAUXCMD
in the proof of }\autoref{thm:epi}\DIFadd{.
	}\item
		\DIFadd{Proof of }\autoref{cor:nondec:onehaus}\DIFadd{: $\mf a^-$ and $\mf a^-_n$ are shifted by $1$ to ensure $\mf a^- \in (0,\infty)$, as required by }\textsc{\DIFadd{LowerBound}}\DIFadd{.
	}\item
		\autoref{thm:alt:convOfMini}\DIFadd{: the convergence of the infima additionally requires $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n$ large enough; the corresponding part of the proof has been corrected.
	}\item
		\DIFadd{Proof of }\autoref{thm:ULLN}\DIFadd{: the centers of the covering balls are chosen in $B$ and the inner suprema are restricted to $B$.
	}\item
		\DIFadd{Proof of }\autoref{lmm:nondec:existence}\DIFadd{: the divergence of $\tilde h$ and the validity of $\tilde h\pr(t) = g(t)/(1-F(t))$ for almost all $t$ are now justified correctly, and $a_{n+1}$ instead of $a_n$ is the derivative of $h$ on $(x_n, x_{n+1})$.
	}\item
		\DIFadd{Proof of }\autoref{lmm:precompact}\DIFadd{: replaced by an argument via total boundedness.
	}\item
		\DIFadd{Appendix \ref{sec:median}: the bound on $\PrOf{\abs{p_n-\frac{1}{2}} > \frac{1}{2}\epsilon_n}$ obtained from Markov's inequality was incorrect; Hoeffding's inequality is used instead.
	}\item
		\DIFadd{Several typographical errors, in particular $H(y) = 0$ instead of $H(0) = 0$ in the proof of }\autoref{lmm:nondec} \DIFadd{and $\abs{f(Y,p) - f(Y,p)}$ instead of $\abs{f(Y,q) - f(Y,p)}$ in the proof of }\autoref{thm:ULLN}\DIFadd{.
	}\end{enumerate}
\end{remark}
 \DIFaddend \end{appendix}
