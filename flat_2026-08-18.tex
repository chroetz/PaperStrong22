...
The following generalizations are considered: 
...
\cite{huckemann11} shows almost sure convergence in one-sided Hausdorff distance as well as in outer limit for generalized Fréchet means. Our results for $\mf c$-Fréchet means require slightly less strict assumptions, see \autoref{thm:epi} and \autoref{thm:consistency}, which make them applicable in a larger class of settings and allows us to derive our results for $H$- and $\alpha$-Fréchet means with minimal moment assumptions.
...
When $M$ is a singleton, a quantitative version (rates of convergence) of the results presented in this article is given in \cite{schoetz19}.
...
All strong laws in the main part of this article build upon \cite[Theorems 1.1 and 8.2]{korf01} -- deep convergence results for functions $\MS \to \R$. In appendix \ref{sec:alt}, we show a different route to a strong law in one-sided Hausdorff distance. This is illustrative, but requires slightly stricter assumptions. 
...
We want to show that there is a bounded set $B_1 \subset \mc Q$ and a random variable $N_1$ such that $F(q) \geq \inf F + 1$ and $F_n(q) \geq \inf F_n + 1$ for all $q\in \mc Q \setminus B_1$ and all $n \geq N_1$. If $\mc Q$ is bounded, we can take $B_1 = \mc Q$ and $N_1 = 1$. Assume $\mc Q$ is not bounded.
...
As $G_{\mf b^+, \mf b^-}(\delta)\xrightarrow{\delta\to\infty}\infty$, we can choose $\delta$ large enough such that $G_{\mf a^+, \mf a^-}(\delta) \geq F(o) + 1$ and $G_{\frac12\mf a^+, \mf a^-+1}(\delta) \geq F(o) + 2$. We set $B_1 = \ball_\delta(o)$.
...
\end{corollary}
\begin{proof}
	We check the conditions of \autoref{thm:epi}, with \textsc{IntegrableInf} weakened as described below. \textsc{Polish} is an assumption. 
	...
	Of \textsc{IntegrableInf}, we only need the local version
	\begin{equation*}
		\Ex*{\inf_{q\in B}\br{H(\ol Yq) - H(\ol Yo)}} > -\infty
		\quad\text{for every bounded set } B\subset\mc Q
		\eqcm
	\end{equation*}
	because one can follow the proof of \autoref{thm:epi} with \cite[Theorem 1.1]{korf01} replaced by \cite[Theorem 8.2]{korf01}, which requires only local instead of global inf-integrability.\footnote{\cite[Theorem 8.2]{korf01} is applicable here: the local version of \textsc{IntegrableInf} implies that $\mf c$ is \textit{locally inf-integrable} in the sense of \cite[Definition 4.2]{korf01}, as every $q\in\mc Q$ has a closed bounded neighborhood; a countable subset of $\mc Q\times\R$ that is dense in the epigraph of $F$ exists, as $\mc Q\times\R$ is separable by \textsc{Polish} and every subset of a separable metric space is separable; and the shift on the sequence space of the independent and identically distributed $(Y_i)_{i\in\N}$ is measure preserving and ergodic.} This is necessary here, as \textsc{IntegrableInf} itself is equivalent to $\Ex{H(\ol Yo)} < \infty$ -- since $H\geq0$ and $H(0)=0$, the infimum $\inf_{q\in\mc Q}\br{H(\ol yq) - H(\ol yo)} = -H(\ol yo)$ is attained at $q=y$ -- which is more restrictive than \textsc{$h$-Moment}.
	To show the local version, we note that $H$ is non-decreasing and apply \autoref{lmm:nondec} (iii),
	\begin{align*}
		H(\ol yq) - H(\ol yo)
		&\geq 
	...
	Let $B \subset \mc Q$ be a bounded set and set $D := \sup_{q\in B} \ol qo < \infty$. As $H \geq 0$, the inequality above implies
	\begin{equation*}
		\inf_{q \in B} \br{H(\ol yq) - H(\ol yo)} \geq - 2 D\, h(\ol yo)
		\eqfs
	\end{equation*}
	With that, \textsc{$h$-Moment} implies the local version of \textsc{IntegrableInf}.
	Thus, the conclusion of \autoref{thm:epi} holds.
\end{proof}
...
\end{corollary}
\begin{proof}
	We check the conditions of \autoref{thm:consistency}. \textsc{SampleHeineBorel} and \textsc{Polish} are assumptions of the corollary. \textsc{LowerSemiContinuity} and the local version of \textsc{IntegrableInf} are shown in the proof of \autoref{cor:nondec:epi}; \textsc{IntegrableInf} enters the proof of \autoref{thm:consistency} only through \autoref{thm:epi} in step 1, so that -- as in the proof of \autoref{cor:nondec:epi} -- its local version together with \cite[Theorem 8.2]{korf01} suffices.
	...
	For \textsc{LowerBound}, we use the second inequality. We set $\psi^+(\delta) = 	b^{-1} H(\delta)$, $\psi^-(\delta) = 2 \delta$, $\mf a^+=\mf a_n^+ = 1$, $\mf a^{-} = \Ex{h(\ol Yo)} + 1$, and $\mf a^-_n = \frac1n \sum_{i=1}^n h(\ol {Y_i}o) + 1$ with $\mf a^-_n\xrightarrow{n\to\infty}_{\ms{a.s.}}\mf a^-$ due to \textsc{$h$-Moment}. The added constant $1$ only weakens the lower bounds, as $\psi^-\geq0$, and it guarantees $\mf a^-\in(0,\infty)$ also in the degenerate case $\Ex{h(\ol Yo)} = 0$. Because of the third inequality, $\psi^+(\delta)/\psi^-(\delta) \geq \frac14 b^{-1} h(\frac12 \delta) \xrightarrow{\delta\to\infty} \infty$ by \textsc{InfiniteIncrease}.
\end{proof}
...
At least one of $0$ and $1$ is a minimizer of $F_n(q)=p_n\abs{1-q}+(1-p_n)\abs{q}$ and $F_n$ is linear on $[0,1]$. Thus, $[0,1] \subset M_n$ if and only if  $\epsilon_n \geq |F_n(0)-F_n(1)|$. This is equivalent to
 $|p_n-\frac12| \leq \frac12\epsilon_n$.
As $Y_1,\dots, Y_n$ are independent with values in $[0,1]$ and $\Ex{p_n} = \frac12$, Hoeffding's inequality \cite[Theorem 2]{hoeffding63} yields
\begin{equation*}
	\PrOf{\abs{p_n-\frac12} > \frac12\epsilon_n} \leq 2 \exp\brOf{-\tfrac12 n \epsilon_n^2}
	\eqfs
\end{equation*}
For $\epsilon_n = n^{-\frac14}$, we obtain
\begin{equation*}
	\sum_{n=1}^\infty \PrOf{\abs{p_n-\frac12} > \frac12\epsilon_n}  \leq \sum_{n=1}^\infty 2\exp\brOf{-\tfrac12 \sqrt n} < \infty
	\eqfs
\end{equation*}
The Borel--Cantelli lemma implies that almost surely and for all $n$ large enough,  $|p_n-\frac12| \leq \frac12\epsilon_n$ and thus, $[0,1] \subset M_n$. 
...
\begin{proof}
		...
		Finally, assume $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n$ large enough and let us show the convergence of the infima.
		As $\argmin f \neq \emptyset$, we may pick $y \in \argmin f$. From $f_n(y) \xrightarrow{n\to\infty} f(y) = \inf f$ and $\inf f_n \leq f_n(y)$, we obtain $\limsup_{n\to\infty} \inf f_n \leq \inf f$.
		Assume $\liminf_{n\to\infty} \inf f_n < \inf f$. Then there is $\eta > 0$ such that $\inf f_n < \inf f - 2\eta$ for infinitely many $n$. Let $n_3$ and $B = \bigcup_{n\geq n_3} \epsilon_n\text{-}\argmin f_n$ be as above. As $f_n \xrightarrow{n\to\infty}_{\ubs} f$ and $\diam(B)<\infty$, there is $n_5\in\N$ such that $\sup_{x\in B}\abs{f_n(x) - f(x)} \leq \frac12 \eta$ for all $n \geq n_5$. As $\epsilon_n \xrightarrow{n\to\infty} 0$, there is $n_6\in\N$ such that $\epsilon_n \leq \eta$ for all $n\geq n_6$. By assumption, there is $n_7\in\N$ such that $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n \geq n_7$. For every one of the infinitely many $n \geq \max(n_3, n_5, n_6, n_7)$ with $\inf f_n < \inf f - 2\eta$, choose $x_n \in \epsilon_n\text{-}\argmin f_n$. Then $x_n \in B$ and
		\begin{equation*}
			f(x_n) \leq f_n(x_n) + \tfrac12\eta \leq \inf f_n + \epsilon_n + \tfrac12\eta < \inf f - 2\eta + \eta + \tfrac12\eta < \inf f
			\eqcm
		\end{equation*}
		which is a contradiction. Hence, $\inf f_n \xrightarrow{n\to\infty} \inf f$.
\end{proof}
...
\end{theorem}
\begin{proof}
Define $F(q) = \Ex{\mf c(Y, q)}$, $F_n(q) = \frac1n\sum_{i=1}^n \mf c(Y_i, q)$.  The proof consists of the following steps:
\begin{enumerate}
...
\end{theorem}
\begin{proof}
Let $\epsilon > 0$. As $B$ is compact, the open cover $(\ball_\epsilon(q))_{q \in B}$ of $B$ has a finite subcover, i.e., there is a finite set $\cb{q_1, \dots, q_k} \subset B$ such that $B \subset \bigcup_{\ell=1}^k\ball_\epsilon(q_\ell)$. We split the supremum,
\begin{align*}
...
\begin{align*}
	\lim_{\epsilon\searrow0} 	\Ex*{\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, q) - f(Y, p)} }
	&=
...
\item 
	...
	Hence, $f(x, y) \geq f(x, 0) = 0$, as $H(0) = 0$.
...
\begin{proof}
	...
	\begin{align*}
		...
		\eqfs
	\end{align*}
	As $1-F$ is non-increasing and right-continuous, the definition of $z_n$ implies $1-F(t) \leq 1-F(z_n) \leq \frac1{n-1}$ for all $t\geq z_n$ and $n\geq2$. Thus,
	\begin{align*}
		...
		\eqfs
	\end{align*}
	Moreover, $\tilde h(x)$ is strictly increasing, as $g(t) > 0$ and $1-F(t) > 0$.
	As $1-F$ is non-increasing and positive, the integrand $t \mapsto g(t)/(1-F(t))$ is bounded on every bounded interval. Hence, $\tilde h$ is locally Lipschitz continuous and, in particular, absolutely continuous with $\tilde h\pr(t) = g(t)/(1-F(t))$ for almost all $t\geq0$. Thus,
	\begin{align*}
		\Ex{\tilde h(X)}
		&=
	...
	Furthermore, $a_{n+1} \leq a_n$ as $x_{n+1} \geq x_n + a_n^{-1}$. As $h$ is continuous and $a_{n+1}$ is the derivative of $h$ in the interval $(x_n, x_{n+1})$, $h$ is concave. 
...
\end{equation*}
\end{lemma}
\autoref{lmm:products} can be proven by straightforward calculations.
...
\end{proof}
\begin{remark}[Corrections to the published version]\label{rem:corrections}
	Compared to the published version of this article, the present version corrects the following errors. The numbering of all statements is unchanged.
	\begin{enumerate}[label=(\roman*)]
	\item
		Step 2 of the proof of \autoref{thm:consistency}: the lower bound of \textsc{LowerBound} was evaluated at $\ol qo = \delta$, although no monotonicity of $\psi^+$ or $\psi^-$ is assumed, and the argument used an element $m\in M$ before $M \neq \emptyset$ had been established. The step is now carried out with $G_{\mf b^+, \mf b^-}(\delta) = \inf_{t\geq\delta}\br{\mf b^+ \psi^+(t) - \mf b^- \psi^-(t)}$ and with $\inf F$ and $\inf F_n$ in place of $F(m)$ and $F_n(m)$.
	\item
		Proofs of \autoref{cor:nondec:epi} and \autoref{cor:nondec:onehaus}: \textsc{$h$-Moment} does not imply \textsc{IntegrableInf} -- for the cost function of section \ref{sec:nondec}, \textsc{IntegrableInf} is equivalent to $\Ex{H(\ol Yo)} < \infty$. Only a local version of \textsc{IntegrableInf} is needed, which follows from \textsc{$h$-Moment} and which suffices when \cite[Theorem 1.1]{korf01} is replaced by \cite[Theorem 8.2]{korf01} in the proof of \autoref{thm:epi}.
	\item
		Proof of \autoref{cor:nondec:onehaus}: $\mf a^-$ and $\mf a^-_n$ are shifted by $1$ to ensure $\mf a^- \in (0,\infty)$, as required by \textsc{LowerBound}.
	\item
		\autoref{thm:alt:convOfMini}: the convergence of the infima additionally requires $\epsilon_n\text{-}\argmin f_n \neq \emptyset$ for all $n$ large enough; the corresponding part of the proof has been corrected.
	\item
		Proof of \autoref{thm:ULLN}: the centers of the covering balls are chosen in $B$ and the inner suprema are restricted to $B$.
	\item
		Proof of \autoref{lmm:nondec:existence}: the divergence of $\tilde h$ and the validity of $\tilde h\pr(t) = g(t)/(1-F(t))$ for almost all $t$ are now justified correctly, and $a_{n+1}$ instead of $a_n$ is the derivative of $h$ on $(x_n, x_{n+1})$.
	\item
		Proof of \autoref{lmm:precompact}: replaced by an argument via total boundedness.
	\item
		Appendix \ref{sec:median}: the bound on $\PrOf{\abs{p_n-\frac12} > \frac12\epsilon_n}$ obtained from Markov's inequality was incorrect; Hoeffding's inequality is used instead.
	\item
		Several typographical errors, in particular $H(y) = 0$ instead of $H(0) = 0$ in the proof of \autoref{lmm:nondec} and $\abs{f(Y,p) - f(Y,p)}$ instead of $\abs{f(Y,q) - f(Y,p)}$ in the proof of \autoref{thm:ULLN}.
	\end{enumerate}
\end{remark}
 \end{appendix}
