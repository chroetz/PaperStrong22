...
%DIF DEL .\flat_old.tex   Sun Aug  2 20:44:11 2026
%DIF ADD .\flat_new.tex   Sun Aug  2 20:40:43 2026
...
}\DIFaddend -- a deep convergence result for functions $\MS \to \R$. In appendix \ref{sec:alt}, we show a different route to a strong law in \DIFdelbegin \DIFdel{one-side }\DIFdelend \DIFaddbegin \DIFadd{one-sided }\DIFaddend Hausdorff distance. This is illustrative, but requires slightly stricter assumptions. 
...
\item
	\textsc{IntegrableInf}:
	\DIFdelbegin \DIFdel{$\Ex{\inf_{q \in \mc Q}\mf c(Y, q)} > - \infty$}\DIFdelend \DIFaddbegin \DIFadd{$\Ex{\inf_{q \in B}\mf c(Y, q)} > - \infty$ for every bounded set $B \subset \mc Q$}\DIFaddend .
\end{itemize}
\end{assumptions}
\DIFaddbegin \begin{remark}\label{rem:integrableinf}
	\textsc{\DIFadd{IntegrableInf}} \DIFadd{is a local condition. It is implied by the stronger, global requirement $\Ex{\inf_{q \in \mc Q}\mf c(Y, q)} > - \infty$, but it is considerably less restrictive: for the cost function $\mf c(y,q) = H(\ol yq) - H(\ol yo)$ of section \ref{sec:nondec} with $\mc Y = \mc Q$, the global requirement is equivalent to $\Ex{H(\ol Yo)} < \infty$ -- as $H\geq0$ and $\mf c(y,y) = -H(\ol yo)$, we have $\inf_{q\in\mc Q}\mf c(y, q) = -H(\ol yo)$ -- whereas the local condition already follows from $\Ex{h(\ol Yo)} < \infty$, see the proof of }\autoref{cor:nondec:epi}\DIFadd{.
}\end{remark}
\DIFaddend \begin{theorem}\label{thm:epi}
...
\cite[Theorem 8.2]{korf01} }\hskip0pt%DIFAUXCMD
}\DIFaddend states that $F_n \xrightarrow{n\to\infty}_{\ms{epi}} F$ almost surely if \textsc{Polish}, \textsc{LowerSemiContinuity}, and \textsc{IntegrableInf} are true\DIFaddbegin \DIFadd{: }\textsc{\DIFadd{IntegrableInf}} \DIFadd{implies that $\mf c$ is }\textit{\DIFadd{locally inf-integrable}} \DIFadd{in the sense of \mbox{%DIFAUXCMD
\cite[Definition 4.2]{korf01}}\hskip0pt%DIFAUXCMD
, as every $q\in\mc Q$ has a closed bounded neighborhood; the additional requirement of a countable subset of $\mc Q \times \R$ that is dense in the epigraph of $F$ is automatically fulfilled here, as $\mc Q\times\R$ is separable by }\textsc{\DIFadd{Polish}} \DIFadd{and every subset of a separable metric space is separable; and the shift on the sequence space of the independent and identically distributed $(Y_i)_{i\in\N}$ is measure preserving and ergodic}\DIFaddend . \autoref{thm:convOfMini} then implies $\outerlim_{n\to\infty}\, M_n \subset M$ almost surely.
\end{proof}
\begin{assumptions}\mbox{ }
\begin{itemize}
...
\item 
	...
	\DIFdel{\textsc{HeineBorel} }&\DIFdel{\Rightarrow \textsc{Polish}\eqcm}\\
	\DIFdel{\textsc{HeineBorel} }&\DIFdel{\Rightarrow \textsc{SampleHeineBorel}\eqcm}\\
	\DIFdel{\textsc{UpperBound} }&\DIFdel{\Rightarrow \textsc{Integrable}\eqfs
	}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
	\DIFadd{\textsc{HeineBorel} }&\DIFadd{\Rightarrow \textsc{Polish}\eqcm}\\
	\DIFadd{\textsc{HeineBorel} }&\DIFadd{\Rightarrow \textsc{SampleHeineBorel}\eqcm}\\
	\DIFadd{\textsc{UpperBound} }&\DIFadd{\Rightarrow \textsc{Integrable}\eqcm}\\
	\DIFadd{\textsc{UpperBound} }&\DIFadd{\Rightarrow \textsc{IntegrableInf}\eqfs
	}\end{align*}\DIFaddend 
...
We want to show that there is a bounded set $B_1 \subset \mc Q$ such that \DIFdelbegin \DIFdel{$F(q) \geq F(m) + 1$ and $F_n(q) \geq F_n(m) + 1$ }\DIFdelend \DIFaddbegin \DIFadd{$F(q) \geq \inf F + 1$ and $F_n(q) \geq \inf F_n + 1$ }\DIFaddend for all $q\in \mc Q \setminus B_1$ and \DIFdelbegin \DIFdel{$m \in M$}\DIFdelend \DIFaddbegin \DIFadd{all $n$ large enough}\DIFaddend . If $\mc Q$ is bounded, we can take $B_1 = \mc Q$ \DIFaddbegin \DIFadd{and $N_1 = 1$}\DIFaddend . Assume $\mc Q$ is not bounded.
...
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{$F(o) < \infty$. For $\mf b^+, \mf b^- \in(0,\infty)$ and $\delta \geq 0$ set
}\begin{equation*}
...
\DIFadd{As $\psi^\pm \geq 0$, we have $\mf b^+ \psi^+(t) - \mf b^- \psi^-(t) \geq \max(1,\psi^-(t)) \br{\mf b^+ \frac{\psi^+(t)}{\max(1,\psi^-(t))} - \mf b^-}$ for all $t\geq0$. Since $\psi^+(t)/\max(1, \psi^-(t))\xrightarrow{t\to\infty}\infty$ and $\max(1,\psi^-(t))\geq1$, this implies
}\begin{equation}\DIFadd{\label{eq:cons:G}
	G_{\mf b^+, \mf b^-}(\delta) \xrightarrow{\delta\to\infty} \infty
	\eqfs
}\end{equation}
\DIFadd{Note that no monotonicity of $\psi^+$ or $\psi^-$ is assumed, so it is essential to take the infimum over all $t \geq \delta$ here.
}\DIFaddend Due to \textsc{LowerBound} \DIFdelbegin \DIFdel{, $F(q) \geq \mf a^+ \psi^+(\delta) - \mf a^- \psi^-(\delta) \geq  F(m) + 2$ }\DIFdelend \DIFaddbegin \DIFadd{and $\ol qo \geq \delta$ }\DIFaddend for \DIFaddbegin \DIFadd{$q \in \mc Q\setminus\ball_\delta(o)$, we obtain $F(q) \geq G_{\mf a^+, \mf a^-}(\delta)$ for }\DIFaddend all $q\in\mc Q\setminus \ball_\delta(o)$\DIFdelbegin \DIFdel{and $\delta$ large enough.
This holds for all $m\in M$ as $F(m)$ does not change with $m$. We set $B_1 = \ball_\delta(o)$. For $F_n$, it holds $F_n(m) \xrightarrow{n\to\infty}_{\ms{a.s.}} F(m)$ and $\inf_{q\in\mc Q \setminus B_1} F_n(q) \geq \mf a^+_n \psi^+(\delta) - \mf a^-_n \psi^-(\delta)$ with $\mf a^+_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^+$ and }\DIFdelend \DIFaddbegin \DIFadd{.
As $\mf a^+_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^+ > 0$ and }\DIFaddend $\mf a^-_n \xrightarrow{n\to\infty}_{\ms{a.s.}} \mf a^-$\DIFdelbegin \DIFdel{.
Thus}\DIFdelend , there is a random variable $N_1$ such that almost surely \DIFdelbegin \DIFdel{$F_n(q) \geq F_n(m) + 1$ for all }\DIFdelend \DIFaddbegin \DIFadd{$\mf a^+_n \geq \frac{1}{2} \mf a^+$ and $\mf a^-_n \leq \mf a^- +1$ for all $n\geq N_1$; enlarging $N_1$ if necessary, we may also assume $F_n(o) \leq F(o) + 1$ for all }\DIFaddend $n \geq N_1$, \DIFaddbegin \DIFadd{which is possible by the strong law of large numbers and }\textsc{\DIFadd{Integrable}}\DIFadd{. Hence, almost surely, $F_n(q) \geq G_{\frac{1}{2}\mf a^+, \mf a^-+1}(\delta)$ for all $n\geq N_1$ and all $q\in\mc Q\setminus \ball_\delta(o)$.
By \eqref{eq:cons:G}, we can choose $\delta$ large enough such that $G_{\mf a^+, \mf a^-}(\delta) \geq F(o) + 1$ and $G_{\frac{1}{2}\mf a^+, \mf a^-+1}(\delta) \geq F(o) + 2$. We set $B_1 = \ball_\delta(o)$.
...
\begin{proof}
	...
	\DIFaddend With that, \textsc{$h$-Moment} implies \textsc{IntegrableInf}.
...
As $Y_1,\dots, Y_n$ are independent with values in $[0,1]$ and $\Ex{p_n} = \frac{1}{2}$, Hoeffding}\DIFaddend 's inequality \DIFdelbegin \begin{displaymath}
	\DIFdel{\PrOf{\abs{p_n-\frac{1}{2}} \geq \frac{1}{2}\epsilon_n} \leq \frac{n^{-3} \Ex*{(Y-\frac{1}2)^4}}{2^{-4}\epsilon_n^4}
	\eqfs
...
yields
	...
	\DIFadd{\PrOf{\abs{p_n-\frac{1}{2}} > \frac{1}{2}\epsilon_n} \leq 2 \exp\brOf{-\tfrac12 n \epsilon_n^2}
	\eqfs
}\end{equation*}\DIFaddend 
For $\epsilon_n = n^{-\frac{1}{4}}$, we obtain
\DIFdelbegin \begin{displaymath}
	\DIFdel{\sum_{n=1}^\infty \PrOf{\abs{p_n-\frac{1}{2}} \geq \frac{1}{2}\epsilon_n}  \leq \sum_{n=1}^\infty n^{-2} < \infty
	\eqfs
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
	\DIFadd{\sum_{n=1}^\infty \PrOf{\abs{p_n-\frac{1}{2}} > \frac{1}{2}\epsilon_n}  \leq \sum_{n=1}^\infty 2\exp\brOf{-\tfrac12 \sqrt n} < \infty
	\eqfs
}\end{equation*}\DIFaddend 
...
In contrast to the arguments in the main part of the article, it does not rely on the powerful result \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite[Theorem 1.1]{korf01}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[Theorem 8.2]{korf01}}\hskip0pt%DIFAUXCMD
}\DIFaddend , which seems to be rather complex to prove. Instead our reasoning here is simpler and more self-contained.
...
\begin{theorem}\label{thm:alt:convOfMini}
	...
	Then
	\DIFdelbegin \begin{displaymath}
		\DIFdel{d_\subset(\epsilon_n\text{-}\argmin f_n, \argmin f)\xrightarrow{n\to\infty}
		0
	}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation*}
		\DIFadd{d_\subset(\epsilon_n\text{-}\argmin f_n, \argmin f)\xrightarrow{n\to\infty}
		0
		\eqfs
	}\end{equation*}\DIFaddend 
...
\DIFdelbegin \begin{align*}
	&\DIFdel{\sup_{q\in B} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q) - \Ex{f(Y, q)}}
	}\\&\DIFdel{\leq
	\sup_{\ell \in \cb{1,\dots, k}} 
	\sup_{q\in \ball_\epsilon(q_\ell)}  
		\abs{\frac{1}n \sum_{i=1}^n \br{f(Y_i, q) - f(Y_i, q_\ell)} - \Ex{f(Y, q) - f(Y, q_\ell)}} 
	}\\&\DIFdel{\quad+ 
	\sup_{\ell \in \cb{1,\dots, k}} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q_\ell) - \Ex{f(Y, q_\ell)}}
	\eqfs
}\end{align*}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{align*}
	&\DIFadd{\sup_{q\in B} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q) - \Ex{f(Y, q)}}
	}\\&\DIFadd{\leq
	\sup_{\ell \in \cb{1,\dots, k}} 
	\sup_{q\in B \cap \ball_\epsilon(q_\ell)}
		\abs{\frac{1}n \sum_{i=1}^n \br{f(Y_i, q) - f(Y_i, q_\ell)} - \Ex{f(Y, q) - f(Y, q_\ell)}}
	}\\&\DIFadd{\quad+ 
	\sup_{\ell \in \cb{1,\dots, k}} \abs{\frac{1}n \sum_{i=1}^n f(Y_i, q_\ell) - \Ex{f(Y, q_\ell)}}
	\eqfs
...
\DIFdelbegin \begin{align*}
	&\DIFdel{\sup_{\ell \in \cb{1,\dots, k}} 
	\sup_{q\in \ball_\epsilon(q_\ell)}  
		\abs{\frac{1}n \sum_{i=1}^n \br{f(Y_i, q) - f(Y_i, q_\ell)} - \Ex{f(Y, q) - f(Y, q_\ell)}} 
	}\\& \DIFdel{\leq
...
\DIFdelend \DIFaddbegin \begin{align*}
	&\DIFadd{\sup_{\ell \in \cb{1,\dots, k}} 
	\sup_{q\in B \cap \ball_\epsilon(q_\ell)}
		\abs{\frac{1}n \sum_{i=1}^n \br{f(Y_i, q) - f(Y_i, q_\ell)} - \Ex{f(Y, q) - f(Y, q_\ell)}}
	}\\& \DIFadd{\leq
	\frac{1}n \sum_{i=1}^n \sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y_i, q) - f(Y_i, p)} 
...
\DIFdelbegin \begin{align*}
	\DIFdel{\lim_{\epsilon\searrow0} 	\Ex*{\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, p) - f(Y, p)} }
	}&\DIFdel{=
...
\DIFdelend \DIFaddbegin \begin{align*}
	\DIFadd{\lim_{\epsilon\searrow0} 	\Ex*{\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, q) - f(Y, p)} }
	}&\DIFadd{=
	\Ex*{\lim_{\epsilon\searrow0}\sup_{q,p\in B,\, \sol qp \leq \epsilon} \abs{f(Y, q) - f(Y, p)} }
	=
	0
	\eqfs
}\end{align*}\DIFaddend 
...
\begin{proof}
	...
	\DIFdelbegin \DIFdel{Then
		...
		\DIFdel{\lim_{x\to\infty} \tilde h(x) 
		= 
...
\DIFdelend \DIFaddbegin \DIFadd{As $1-F$ is non-increasing and right-continuous, the definition of $z_n$ implies $1-F(t) \leq 1-F(z_n) \leq \frac{1}{n-1}$ for all $t\geq z_n$ and $n\geq2$. Thus,
		...
		\DIFadd{\lim_{x\to\infty} \tilde h(x)
		=
		\int_0^\infty \frac{g(t)}{1-F(t)} \dl t
		\geq
		\sum_{n=2}^\infty (n-1) n^{-2}
		=
		\infty
		\eqfs
	}\end{align*}\DIFaddend 
...
\begin{proof}
	...
	\DIFdelbegin \DIFdel{Let $\delta_n = 2^{-n}$. Define $B_n := \mc Q \setminus \ball_{\delta_n}(\mc Q \setminus A)$. The sets $B_n$ are closed and subsets of }\DIFdelend \DIFaddbegin \DIFadd{Assume }\DIFaddend $A$ \DIFaddbegin \DIFadd{is not totally bounded. Then there is $\epsilon > 0$ such that no finite collection of balls of radius $\epsilon$ covers $A$}\DIFaddend . \DIFdelbegin \DIFdel{Thus, they are compact. Let $b_k^n\in\argmin_{b \in B_n} d(b, a_k)$. Such an element exists as $B_n$ is compact. Furthermore, $d(b_k^n, a_k) \leq \delta_n$. Define the subindex sequences $(k(n,\ell))_{\ell\in\N} \subset \N$ such that $k(0, \ell) = \ell$ }\DIFdelend \DIFaddbegin \DIFadd{Hence, we can recursively choose $a_1 \in A$ }\DIFaddend and \DIFdelbegin \DIFdel{$(b^n_{k(n, \ell)})_{\ell\in\N}$ is a converging subsequence of $(b^n_{k(n-1, \ell)})_{\ell\in\N}$ with limit $b^n_{k(n, \ell)} \xrightarrow{\ell\to\infty}b^n_\infty$ and $d(b^n_{k(n, \ell)}, b^n_\infty) \leq \delta_\ell$. By the triangle inequality $d(b^{n_1}_k, b^{n_2}_k) \leq d(b^{n_1}_k, a_k) + d(b^{n_2}_k, a_k) \leq \delta_{n_1}+\delta_{n_2}$. Thus, $d(b^{n_1}_\infty, b^{n_2}_\infty) \leq \delta_{n_1}+\delta_{n_2}$}\DIFdelend \DIFaddbegin \DIFadd{$a_{k+1} \in A \setminus \bigcup_{j=1}^{k}\ball_\epsilon(a_j)$ for $k \in \N$. The resulting set $S = \Set{a_k \given k \in\N}$ is infinite and $\epsilon$-separated, i.e., $d(a_j, a_k) \geq \epsilon$ for all $j \neq k$. In particular, every ball of radius $\frac{1}{2}\epsilon$ contains at most one element of $S$. Thus}\DIFaddend , \DIFaddbegin \DIFadd{$S$ has no point of accumulation in $\mc Q$, }\DIFaddend which makes \DIFdelbegin \DIFdel{$(b^{n}_\infty)_{n\in\N}$ a Cauchy-sequence. Define $a$ as its limit, i.e., $b^{n}_\infty \xrightarrow{n\to\infty} a$. As $b^{n}_\infty\in B_n \subset\overline{A}$, also $a\in\overline{A}$.
...
%DIFDELCMD < 	%%%
\DIFdel{i.e., $(a_{k(n, n)})_{n\in\N}$ is subsequence of $(a_k)_{k\in\N}$ which converges in $\overline{A}$}\DIFdelend \DIFaddbegin \DIFadd{$S$ a closed subset of $\mc Q$, and the cover of $S$ by the balls $\ball_{\frac{1}{2}\epsilon}(a_k)$, $k\in\N$, has no finite subcover, which makes $S$ non-compact. As $S \subset A$, this contradicts the assumption of the lemma.
}

	\DIFadd{Hence, $A$ is totally bounded and $\overline A$ is compact}\DIFaddend .
\end{proof}
 \end{appendix}
